...
# libxml2 is required for XML API support (especially OEM types)
try:
    import libxml2
except ImportError:
    print("ERROR: libxml2 is required but not installed.", file=sys.stderr)
    print("Please install the python3-libxml2 package:", file=sys.stderr)
    print("  Debian/Ubuntu: sudo apt install python3-libxml2", file=sys.stderr)
    print("  RHEL/Fedora:   sudo dnf install python3-libxml2", file=sys.stderr)
    print("  openSUSE:      sudo zypper install python3-libxml2", file=sys.stderr)
    sys.exit(1)
...
def query_oem_type_raw(type_id):
    ...
    # Try XML API for full data extraction
    try:
        xml_api = dmidecode.dmidecodeXML()
        xml_api.SetResultType(dmidecode.DMIXML_DOC)
        xml_doc = xml_api.QueryTypeId(type_id)
...
def extract_data_from_xml_string(xml_str, type_id):
    ...
    The XML format from dmidecode can be:
    1. For supported types with raw data:
       - <Handle handle="0x0048" type="130" size="39">
       -   <HeaderAndData><Row index="0">0x020x03...</Row></HeaderAndData>
       -   <Strings><String index="1">some string</String></Strings>
       - </Handle>

    2. For unsupported OEM types (no decoder in dmidecode):
       - <DMImessage type="130" unsupported="1" handle="0x0048" size="39">
       -   DMI/SMBIOS type 0x82 is not supported by dmidecode
       - </DMImessage>
       Note: These types still exist in the SMBIOS table but dmidecode
       cannot decode them. The size indicates the structure exists.
    """
    import re
    result = {}
    ...
    # These indicate OEM types that dmidecode recognizes but can't decode
    dmimessage_pattern = rf'<DMImessage[^>]*type="{type_id}"[^>]*unsupported="1"[^>]*handle="(0x[0-9A-Fa-f]+)"[^>]*size="(\d+)"[^>]*>([^<]*)</DMImessage>'
    dmimessages = re.findall(dmimessage_pattern, xml_str)

    if dmimessages:
        # Found unsupported OEM type entries
        for handle, size, message in dmimessages:
            entry = {
                ...
                'data': {
                    ...
                    'Message': message.strip() if message.strip() else f'OEM type {type_id} (0x{type_id:02X}) not decoded'
                }
            }
            result[handle] = entry
        return result

    # Find all handles in the XML (for supported types with full data)
